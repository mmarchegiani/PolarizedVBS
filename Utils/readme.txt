...
root2pandas_multiproc.py is a Python script to read the .root file exploiting multiprocessing:
1) Four arguments are required:
     python root2pandas_multiproc.py [filename] [part_type] [label] [Nchunks]
2) Save the MadGraph output TTree into a Pandas dataframe using 20 processors:
     python root2pandas_multiproc.py zz_events.root final zz_mumuee 20

pandas2plots_multiproc.py is a Python script to read the Pandas dataframe and produce plots exploiting multiprocessing:
1) Four arguments are required:
python pandas2plots_multiproc.py [filename] [label] [plot_card] [Nchunks]
